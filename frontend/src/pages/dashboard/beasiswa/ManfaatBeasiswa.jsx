...
export default function ManfaatBeasiswa() {
  const [data, setData] = useState([]);
  const [beasiswas, setBeasiswas] = useState([]);
  const [form, setForm] = useState({ beasiswa_id: "", manfaat: "" });
  const [editId, setEditId] = useState(null);

  // ================= GET DATA =================
  const getData = async () => {
    try {
      const res = await api.get("/manfaat-beasiswa");
      setData(res.data);
    } catch (err) {
      console.error("Error get manfaat:", err);
    }
  };
  ...
  const getBeasiswa = async () => {
    try {
      ...
      console.error("Error get beasiswa:", err);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        await api.put(`/manfaat-beasiswa/${editId}`, form);
        setEditId(null);
      } else {
        await api.post("/manfaat-beasiswa", form);
      }
      setForm({ beasiswa_id: "", manfaat: "" });
      getData();
    } catch (err) {
  ...
  const handleEdit = (item) => {
    setForm({ beasiswa_id: item.beasiswa_id, manfaat: item.manfaat });
    setEditId(item.id);
  };
  ...
  const handleDelete = async (id) => {
    if (confirm("Yakin hapus?")) {
      try {
        await api.delete(`/manfaat-beasiswa/${id}`);
        getData();
      } catch (err) {
    ...
    <div className="min-h-screen bg-slate-50 p-6">
      ...
      <div className="flex items-end gap-3 mb-8 pb-5 border-b border-blue-100">
        ...
        <div>
          ...
          <h1 className="text-2xl font-bold text-blue-900 leading-none tracking-tight">Manfaat Beasiswa</h1>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-blue-100 shadow-sm p-6 mb-8 max-w-2xl">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h2 className="text-sm font-semibold text-blue-900 tracking-wide uppercase">
            {editId ? "Edit Manfaat" : "Tambah Manfaat"}
          </h2>
          {editId && (
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div>
            ...
            <input
              type="text"
              name="manfaat"
              placeholder="Masukkan manfaat beasiswa..."
              value={form.manfaat}
              onChange={handleChange}
              required
          ...
          <div className="flex gap-3 pt-2 border-t border-blue-50">
            ...
            {editId && (
              <button
                type="button"
                onClick={() => { setEditId(null); setForm({ beasiswa_id: "", manfaat: "" }); }}
      ...
      <div className="bg-white rounded-2xl border border-blue-100 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-blue-50 flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-blue-900 tracking-wide uppercase">Daftar Manfaat</h2>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gradient-to-r from-blue-900 to-blue-700">
                ...
                <th className="px-5 py-3.5 text-left text-xs font-medium tracking-widest uppercase text-white/70">Manfaat</th>
              ...
              {data.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-5 py-12 text-center text-blue-300 text-sm font-medium">
                    Belum ada data manfaat beasiswa
                  </td>
                </tr>
              ) : (
                data.map((item, index) => (
