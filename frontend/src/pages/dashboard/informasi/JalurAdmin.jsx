...
export default function JalurAdmin() {
  ...
  const fetchData = async () => {
    const res = await api.get("/jalur-admin");
    setData(res.data);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (editId) {
      await api.put(`/jalur-admin/${editId}`, form);
    } else {
      await api.post("/jalur-admin", form);
    }

    setForm({ title: "", description: "", icon: "", gradient: "" });
    setEditId(null);
    fetchData();
  };
  ...
  const handleDelete = async (id) => {
    await api.delete(`/jalur-admin/${id}`);
    fetchData();
  };
  ...
  const fields = [
    { name: "title",       label: "Judul",    placeholder: "contoh: Jalur Reguler",                  type: "input"    },
    { name: "icon",        label: "Icon",     placeholder: "contoh: Award, FileCheck, dll",           type: "input"    },
    { name: "gradient",    label: "Gradient", placeholder: "contoh: from-cyan-400 to-blue-500",       type: "input"    },
    { name: "description", label: "Deskripsi",placeholder: "Tulis deskripsi jalur pendaftaran...",    type: "textarea" },
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6">
      ...
      <div className="flex items-end gap-3 mb-8 pb-5 border-b border-blue-100">
        ...
        <div>
          <p className="text-xs font-medium tracking-widest text-blue-400 uppercase mb-0.5">Manajemen Pendaftaran</p>
          <h1 className="text-2xl font-bold text-blue-900 leading-none tracking-tight">Admin Jalur Pendaftaran</h1>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-blue-100 shadow-sm p-6 mb-8">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h2 className="text-sm font-semibold text-blue-900 tracking-wide uppercase">
            {editId ? "Edit Data Jalur" : "Tambah Data Jalur"}
          </h2>
          {editId && (
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex gap-3 pt-4 border-t border-blue-50">
            ...
            {editId && (
              <button
                type="button"
                onClick={() => { setForm({ title: "", description: "", icon: "", gradient: "" }); setEditId(null); }}
      ...
      <div className="bg-white rounded-2xl border border-blue-100 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-blue-50 flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-blue-900 tracking-wide uppercase">Daftar Jalur Pendaftaran</h2>
              ...
              {data.length === 0 ? (
                ...
                data.map((item, index) => (
                  <tr key={item.id} className="hover:bg-blue-50/50 transition-colors duration-100">
                    ...
                    <td className="px-4 py-3.5 text-blue-700 whitespace-nowrap">{item.icon}</td>
                    <td className="px-4 py-3.5">
